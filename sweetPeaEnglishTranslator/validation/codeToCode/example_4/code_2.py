...
def is_task_transition_forced_first(task):
    return task[0] != "free choice task" and task[1] == "free choice task"
task_transition = factor("task transition", [derived_level("forced switch", transition(is_task_transition_forced_switch, [task])), derived_level("forced repeat", transition(is_task_transition_forced_repeat, [task])), derived_level("free transition", transition(is_task_transition_free_transition, [task])), derived_level("free repeat", transition(is_task_transition_free_repeat, [task])), derived_level("forced first", transition(is_task_transition_forced_first, [task]))])
### EXPERIMENT
